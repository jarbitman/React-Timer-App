...
import { formatTime, padNumber } from '../helpers.js';
...
class Timer extends React.Component {
  ...
  deleteLap = (e) => {
    this.newLapArray = this.filterLapArray(this.state.laps, e.target.closest(".lap").dataset.lapTime.value);
    this.setState(() => ({
  ...
  Laps = () => {
    ...
    return this.state.laps.map(
        (lapTime, i) =>
            <li className="lap" data-lapTime={ lapTime } key={ 'lap' + (i + 1).toString() }>
              Lap { padNumber(i + 1, padLapsTo) }: { lapTime } <FaTrashAlt onClick={ this.deleteLap }
                                                                           id={ 'lap' + (i + 1).toString() }
                                                                           laptime={ lapTime }/>
            </li>,
    );
  ...
  render = () => {
    if (this.state.isVisible) {
      return (
          <div>
            <div className="Timer">{ formatTime(this.getCurrentTime()) } <FaClock /></div>
            <br /><br />
            <this.StartButton />
            <this.StopButton />
            <this.LapButton />
