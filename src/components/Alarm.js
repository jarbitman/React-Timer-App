...
class Alarm extends React.Component {
  ...
  addAlarm = () => {
    if (this.state.alarms.indexOf(this.state.currentAlarm) === -1) {
      ...
      this.setState((prevState) => ({
        ...
        timeouts: [...prevState.timeouts, setTimeout(() => {
          this.alertUser();
        }, prevState.currentAlarm - new Date())],
      }));
    } else {
  ...
  render = () => {
    ...
    if (this.state.isVisible) {
      return (
          <div>
            <DateTimePicker onChange={ this.updateAlarmTime } value={ this.state.currentAlarm } calendarType="Hebrew"/>
            <FaPlus onClick={ this.addAlarm }/>
